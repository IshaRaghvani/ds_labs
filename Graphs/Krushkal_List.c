...
void initUnionFind(struct uniFind* uf,int vertex){  //initializes parent and rank
...
int find(struct uniFind *uf, int v){  //find parent of each vertex
    if(uf->parent[v]!=v){
...
void Union(struct uniFind *uf, int vertex1,int vertex2){ // finding parent between two nodes
    int u=find(uf,vertex1); 
    int v=find(uf,vertex2); 
    if(uf->rank[u]<uf->rank[v]){  //greater rank becomes the parent
        uf->parent[u]=v;
    }
